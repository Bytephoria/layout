...
public final class PacketLayoutMenu extends AbstractContainerMenu implements PacketMenu {
    ...
    public PacketLayoutMenu(
        ...
        //  - layout (container) slots are locked when all clicks or layout clicks are canceled.
    ...
    @Override
    public void clicked(final int slotIndex, final int buttonNum, final @NotNull ClickType clickType, final @NotNull Player player) {
        ...
        final ItemClickType itemClickType = ClickTypeConverter.fromNms(clickType, buttonNum);
        final int hotbarButton = itemClickType == ItemClickType.NUMBER_KEY ? buttonNum : -1;
        this.layout.handleMenuClick(this.bukkitPlayer, slotIndex, itemClickType, hotbarButton);
    }
