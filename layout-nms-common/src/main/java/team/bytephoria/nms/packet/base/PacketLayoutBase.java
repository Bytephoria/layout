...
public abstract class PacketLayoutBase implements Layout {
    ...
    private static PacketMenuFactory menuFactory;

    public static void setPlugin(final @NotNull JavaPlugin javaPlugin) {
        plugin = javaPlugin;
    }

    /** Set once at plugin startup via {@code PacketLayout#init}, to the adapter matching this server's NMS. */
    public static void setMenuFactory(final @NotNull PacketMenuFactory factory) {
        menuFactory = factory;
    }
    ...
    protected abstract @NotNull PacketContainerKind containerKind();
    ...
    public void open(final @NotNull Player player) {
        if (menuFactory == null) {
            throw new IllegalStateException(
                    "PacketLayout.init(plugin, menuFactory) was never called — pass the PacketMenuFactory "
                            + "from whichever layout-nms-adapter-* module matches this server's Paper version."
            );
        }
        ...
        serverPlayer.openMenu(new MenuProvider() {
            ...
            public @NotNull AbstractContainerMenu createMenu(final int containerId, final @NotNull Inventory inventory, final @NotNull net.minecraft.world.entity.player.Player p) {
                return menuFactory.create(containerKind(), containerId, inventory, size(), PacketLayoutBase.this, player);
            }
        });

        // openMenu only assigns containerMenu if the menu was actually opened (e.g. not cancelled).
        if (!(serverPlayer.containerMenu instanceof PacketMenu menu)) {
            return;
        }
    ...
    /**
     * Invoked by the adapter's {@link PacketMenu} when the player clicks a slot. Visual-only: this
     * runs the item's {@link Executable} and the behavior callbacks but never moves any item.
     */
    ...
    /**
     * Invoked by the adapter's {@link PacketMenu} when the menu is closed (by the player, the
     * server, or because another menu replaced it).
     */
