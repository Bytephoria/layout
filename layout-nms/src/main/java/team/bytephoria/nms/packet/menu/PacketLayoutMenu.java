...
import team.bytephoria.nms.packet.render.PacketClickTypeConverter;

import java.util.List;

/**
 * A real server-side container menu backing a visual-only layout. All slots are locked
 * (no pickup, no placement, no shift-move), so the vanilla click pipeline rejects every
 * item movement — including offhand swaps and number-key swaps — and re-syncs the client
 * automatically via {@link #broadcastChanges()}.
 *
 * <p>Because the server now owns a menu whose {@code containerId} matches the client window,
 * the client's optimistic predictions are properly acknowledged and reverted, exactly like
 * a vanilla GUI with cancelled clicks. No packet faking or container reopening is required.
 */
public final class PacketLayoutMenu extends AbstractContainerMenu {
    ...
    @Override
    public void clicked(final int slotIndex, final int buttonNum, final @NotNull ClickType input, final @NotNull Player player) {
        ...
        final ItemClickType clickType = PacketClickTypeConverter.fromNms(input, buttonNum);
